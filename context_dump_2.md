...
### Chat is linked to ConversationAnalysis (1:1)
- DECIDED: Each ConversationAnalysis has its own linked Chat for better UX.
- The AI therapist has context of that specific conversation for discussion.
- This replaces the original "start from suggestion" feature — the whole chat is conversation-scoped.
- `Chat.conversationAnalysisId` references the linked conversation.
...
## Type Implementation Decisions (from model implementation session)

### Type file structure
Types are implemented in `lib/types/`:
```
lib/types/
├── index.ts          # Barrel export
├── emotions.ts       # TODO placeholder (waiting for Hume.ai)
├── conversation.ts   # ConversationPhase, ConversationAnalysis, ConversationEmoji
├── transcript.ts     # SpeakerIdentification, TranscriptSegment, Transcript
├── progress.ts       # EQDimension, ProgressPoint, ImprovementArea, UserProgress
├── user.ts           # User, CoreUserFile, CoreUserFileUpdate
└── chat.ts           # ChatMessage, Chat, ChatListItem
```

### EmotionScores type — DEFERRED
- DECIDED: Do not implement until Hume.ai integration is done.
- `emotions.ts` contains only a TODO comment placeholder.
- All types that reference EmotionScores use `unknown` with TODO comments.
- Once Hume.ai API is integrated, implement based on their actual output format.

### Timestamps are absolute
- All timestamps in the system are absolute: seconds from conversation start.
- This applies to: ConversationPhase.startTime/endTime, TranscriptSegment.startTime/endTime, emotion scores.
- No relative timestamps or message-ID-based references.

### Single timestamp for ConversationAnalysis
- DECIDED: Use only `analyzedAt` (when analysis was created).
- Removed `conversationDate` — we don't reliably know when the conversation happened,
  only when it was uploaded/analyzed.

### Evidence format is quoted excerpts
- ProgressPoint.evidence and ImprovementArea.evidence use quoted excerpts.
- Format: `"'You started by asking about their day' - Oct 15"`
- Not conversation IDs or deep links — actual quotes for human readability.

### Speaker identification failure
- If voice fingerprinting fails to identify the user, show an error.
- TODO comment in `SpeakerIdentification.isUser` for handling uncertain case.
- We hope this doesn't happen; fallback UX not implemented yet.

### Storage not implemented
- DB/RAG storage is NOT implemented in types — leave for other dev.
- RAG is queried from Backboard.
- Types define the shape of data, not where it's stored.
...
- If fingerprinting fails: show error that user is uncertain (TODO — not implemented).
...
Contents (as defined in `CoreUserFile` type):
- `background`: Job, lifestyle, typical day
- `relationships`: Key relationships and dynamics (who they talk to, nature of relationship)
- `goals`: What they want to improve about themselves
- `triggers`: Known triggers and sensitivities
- `eqBaseline`: EQ baseline and growth areas
- `patterns`: Recurring themes and patterns observed across conversations
- `lifeContext`: Important life events/context ("just started new job", "going through breakup")
...
- Each Chat is linked to a specific ConversationAnalysis (1:1 relationship).
- Standard chat UI with message history.
- The AI has access to:
  - Core user file (always loaded as context)
  - The linked ConversationAnalysis for context
...
- The old `lib/data.ts` types are OUTDATED — use `lib/types/` instead.
...
GET    /api/chats
       ...
       Response: ChatListItem[]  (id, conversationAnalysisId, preview, createdAt)
       For the sidebar.

POST   /api/chats
       Body: { conversationAnalysisId: string }
       Response: { chatId: string }
       Creates a new chat session linked to a conversation.
...
- RAG / Vector store: Backboard (queried, not implemented in types)
...
DECIDED: Do not implement until Hume.ai integration. Use `unknown` with TODO comments for now.

Once Hume.ai API is checked, we need to either:
a) Use Hume's categories directly, or
b) Map Hume's output to our own reduced set.

This is a blocking decision for full implementation.
...
- `lib/data.ts` — Old mock data and types. Use `lib/types/` instead.
