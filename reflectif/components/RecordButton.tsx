        ...
        <div className="flex flex-col items-center justify-center gap-8 py-10 relative">
            ...
            <AnimatePresence mode="wait">
                {isRecording ? (
                    ...
                    >
                        <div className="flex items-center gap-2 text-red-400 font-mono text-sm tracking-widest uppercase">
                    ...
                    >
                        <h2 className="text-2xl font-light text-zinc-300 mb-2">Conversation Processed</h2>
                        <div className="flex justify-center mt-6">
                            <Link
                                href={`/conversation/${MOCK_CONVERSATIONS[0].id}`}
                                className="group relative inline-flex items-center gap-3 px-8 py-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full transition-all duration-300 hover:scale-105 hover:shadow-[0_0_30px_rgba(139,92,246,0.2)]"
                            >
                                <span className="text-2xl">😌</span>
                                <div className="text-left">
                                    <div className="text-xs text-zinc-400 uppercase tracking-widest">Analysis Ready</div>
                                    <div className="text-lg font-medium text-white">View Summary</div>
                                </div>
                    ...
                    >
                        ...
                        <p className="text-zinc-400">Press to start a conversation with yourself.</p>
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            {!showAnalysisLink && (
                ...
                >
                    ...
                    {/* Button Surface */}
                    <div className={cn(
                        "relative z-10 w-48 h-48 rounded-full flex items-center justify-center border transition-all duration-500",
                        isRecording
                        ...
                        >
                            {isRecording ? (
                                <FiSquare className="w-12 h-12 text-red-500 fill-current" />
                            ) : (
                                <FiMic className="w-12 h-12 text-zinc-300 group-hover:text-violet-400 transition-colors" />
                            )}
                        </motion.div>
                    </div>
                </button>
